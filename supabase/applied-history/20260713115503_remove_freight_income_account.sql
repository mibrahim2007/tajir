-- Freight Income (4120) was superseded by the freight pass-through (Freight
-- Clearing 1170). It has no balance and no journal lines, so drop it from the
-- seed and remove it from every tenant.
...
-- Remove the account from every tenant (safe: no journal lines reference it).
DELETE FROM chart_of_accounts WHERE system_key = 'freight_income';