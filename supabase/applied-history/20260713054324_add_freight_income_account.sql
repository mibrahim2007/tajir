-- Add a dedicated Freight Income revenue account (4120) so service-item sales
-- (e.g. freight recharged to customers) credit their own account instead of
...
-- Backfill Freight Income for every tenant that already has a chart of accounts
-- but is missing this account, so posting service sales never silently skips.
INSERT INTO chart_of_accounts
  (tenant_id, code, name, account_type, parent_code, is_header, is_system, system_key)
SELECT DISTINCT c.tenant_id, '4120', 'Freight Income', 'revenue', '4000', false, true, 'freight_income'
FROM chart_of_accounts c
WHERE NOT EXISTS (
  SELECT 1 FROM chart_of_accounts x
  WHERE x.tenant_id = c.tenant_id AND x.system_key = 'freight_income'
)
ON CONFLICT (tenant_id, code) DO NOTHING;