...
serve(async (req) => {
  ...
  const { customerId, stockItemId, quantity, rate, date, locationId, paymentDueDate } = body as {
    customerId: string; stockItemId: string; quantity: number; rate: number; date: string; locationId: string; paymentDueDate?: string
  }
  ...
  const { data: serialNumber } = await admin.rpc('next_document_serial', {
    p_tenant_id: tenantId, p_doc_type: 'sale_invoice', p_date: date,
  })
  ...
  await postGl(admin, tenantId, date, 'Sale (Mobile)', 'sale_order', order.id, 'SI', [
