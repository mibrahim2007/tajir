...
--
-- Every agent carries TWO independent commission settings — one for sales they
-- bring, one for purchases they arrange — because the rate and even the basis
-- routinely differ between the two sides of the same broker's book.
--
...
-- so inventory stays at supplier invoice cost and lot costing is untouched):
...
--
-- The agent is carried as the `agent_id` sub-ledger dimension on the payable
-- leg — mirroring customer/supplier/stock/employee/owner — so a per-agent
-- position is a GROUP BY, and enrolling an agent never grows the chart of
-- accounts.
...
create table if not exists agent_commissions (
  ...
  -- 'sale_invoice' | 'purchase_invoice' — matches the journal entry source_type
  -- of the document that generated it.
  source_type     text not null check (source_type in ('sale_invoice','purchase_invoice')),
  source_id       uuid not null,
  document_serial text,
  -- Customer or supplier on the source document, denormalised so the agent
  -- ledger reads without joining across both sales and purchases.
  party_name      text,
...
-- ── 6. Row-level security (same JWT expression as 0025 / 0026 / 0036) ──
...
-- function actually DEPLOYED (not just the copies in this folder — some
-- migrations, including the one that added 1170 Freight Clearing, were applied
-- straight to the database and never written here).
--
-- Each migration that touches this function redefines it wholesale from its own
-- copy, so the deployed version had lost 1135, 1170, 3300 and 3400 along with
-- the retained_earnings and salaries_wages keys that earlier migrations added.
-- Existing tenants kept those rows because each of those migrations backfilled
-- them separately — but every tenant created since has been seeded WITHOUT
-- them. That is a live bug: a new tenant gets no Freight Clearing account, so
-- their first sale invoice carrying a service line fails to post to the GL.
--
-- Rebuilding from the true union fixes new tenants, and re-running the backfill
-- repairs any existing tenant still missing a row.
...
-- Existing tenants already HAVE 6500 (so the insert above was a no-op for them)
-- but with system_key null — set it. Same guarded shape as 0037's 3200 backfill:
-- skip any tenant that has somehow already assigned the key elsewhere.
...
-- Same repair for the keys 0043 dropped, for tenants seeded between 0043 and now.
update chart_of_accounts c
