...
import { fetchCustomers, fetchStockItems, createSale } from '../lib/api'
...
export function QuickSaleScreen() {
  ...
  const reset = () => {
    setCustomer(null); setItem(null); setQuantity(0); setRate(0)
  }
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true)
    try {
      await createSale({ customerId: customer.id, stockItemId: item.id, quantity, rate, date: today() })
