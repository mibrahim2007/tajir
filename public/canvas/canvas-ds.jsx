...
function AppSidebar({active, onNavigate}) {
  ...
  return <aside style={{width:252,minWidth:252,background:"var(--surface)",
    ...
    <nav style={{flex:1,overflow:"auto",padding:"2px 12px"}}>
      {NAV_DATA.map(g=><div key={g.group} style={{marginBottom:16}}>
        ...
        {g.items.map(it=>{const on=it.key===active;const Icon=Ic[it.icon]||Ic.Box;return(
          <div key={it.key} onClick={()=>onNavigate&&onNavigate(it.key)}
            style={{position:"relative",display:"flex",alignItems:"center",gap:11,height:40,
            padding:"0 10px",borderRadius:12,marginBottom:2,cursor:"pointer",
...
function AppFrame({active,title,subtitle,actions,children,onNavigate,w=1300,h=840}) {
