 ...
 *
 * It is transient, never pinned. The backdrop that lifts it off the page also
 * dims and blocks that page, so leaving it open would strand whatever you just
 * navigated to behind a greyed-out sheet. Choosing a destination closes it.
 */
export function AppShell({
  ...
  // Picking something closes the menu, at every width. This used to keep the
  // drawer open on a wide screen so it could be left pinned — but the backdrop
  // that separates it from the page also dims and blocks the page, so the form
  // you just opened sat greyed out behind a menu you had to dismiss by hand.
  // Closing here covers routes reached from anywhere, the command palette
  // included, not only from a click inside the drawer.
  useEffect(() => {
    setOpen(false);
  }, [pathname]);
  ...
  return (
    <div className="min-h-screen">
      ...
      {/* ── Nav drawer ──────────────────────────────────────────── */}
      {/* Transient by design: it opens from the ⋮, and closes on Escape, on a
          backdrop click, or as soon as you choose a destination. */}
      <div
        onClick={() => setOpen(false)}
        aria-hidden
        className={cn(
          "fixed inset-0 top-14 z-30 bg-foreground/25 backdrop-blur-[2px] print:hidden transition-opacity duration-200",
      ...
      <aside
        className={cn(
          ...
          "shadow-[8px_0_24px_-12px_hsl(var(--foreground)/0.25)] transition-transform duration-200 ease-out",
      ...
      >
        <button
          ...
          className="absolute right-2 top-3 h-8 w-8 rounded-lg flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-secondary transition-colors z-10"
        >
          <X className="h-4 w-4" />
        </button>
