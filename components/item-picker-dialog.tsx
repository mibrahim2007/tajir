...
type Props = {
  ...
  title?: string
  disabled?: boolean
  /** Label for the quick-create button, e.g. "New Supplier" */
  createLabel?: string
  /** Called after quick-create succeeds — parent should append the item to its list */
  onCreateSuccess?: (item: PickerItem) => void
  /** Render the quick-create mini-form inside the dialog */
  quickCreate?: (onSuccess: (item: PickerItem) => void, onCancel: () => void) => React.ReactNode
}

export function ItemPickerDialog({
  items, value, onSelect,
  placeholder = 'Select item…', title = 'Select Item',
  disabled = false,
  createLabel, onCreateSuccess, quickCreate,
}: Props) {
  ...
  const showCreate = !!quickCreate && !!createLabel
        ...
        <DialogContent className="max-w-lg p-0 gap-0">
          ...
          {mode === 'list' ? (
              ...
              <div className="max-h-80 overflow-y-auto">
                {filtered.length === 0 ? (
                  <div className="py-10 text-center">
                    <p className="text-sm text-muted-foreground mb-3">No items match your search</p>
                    {showCreate && (
            ...
            <div className="px-4 pt-3 pb-4">
              ...
              {quickCreate?.(handleCreated, () => setMode('list'))}
            </div>
          )}
        </DialogContent>
      </Dialog>
