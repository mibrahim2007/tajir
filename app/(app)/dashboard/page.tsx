...
export default async function DashboardPage() {
  ...
  const transactions: Txn[] = [
    ...(recentSalesData ?? []).map(s => ({
      ...
      type: 'Sale' as const,
      party: customerMap.get(s.customer_id as string) ?? '—',
      amount: parse(s.pkr_equivalent),
    })),
    ...(recentPurchasesData ?? []).map(p => ({
      id: p.id,
      date: p.date as string,
      type: 'Purchase' as const,
      party: supplierMap.get(p.supplier_id as string) ?? '—',
      amount: parse(p.pkr_equivalent),
    })),
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <KpiCard
          label="Receivables"
          value={shortPKR(receivables)}
          sub={receivables > 0 ? 'Outstanding from customers' : 'All settled'}
          up={receivables > 0 ? undefined : undefined}
        />
        <KpiCard
          label="Inventory"
          value={totalInventoryUnits > 0 ? totalInventoryUnits.toLocaleString('en-IN') + ' units' : (inventoryData?.length ?? 0) + ' lots'}
          sub="Stock on hand"
        />
      </div>
