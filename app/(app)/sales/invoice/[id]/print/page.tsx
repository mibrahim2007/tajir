...
import { loadSaleInvoice } from '@/lib/sales/load-sale-invoice'
...
export default async function PrintSaleInvoicePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const invoice = await loadSaleInvoice(invoiceId, { includeCostWarnings: true })
  if (!invoice) notFound()
  // Tenant isolation: the loader keys purely on invoice_id, so verify ownership here.
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="print:hidden flex items-center gap-3 px-6 py-4 border-b bg-background sticky top-0">
        ...
        <span className="text-sm text-muted-foreground flex-1">Sale Invoice · {invoice.voucherNo} · {invoice.lines.length} item{invoice.lines.length !== 1 ? 's' : ''}</span>
