...
export function ItemTypeActions({ id, name }: { id: string; name: string }) {
  ...
  const onEdit = (values: FormValues) => {
    startTransition(async () => {
      setError(null)
      const result = await editItemTypeAction({ id, ...values })
  ...
  const onDelete = () => {
    if (!confirm(`Delete item type "${name}"? Stock items using this type will be unlinked.`)) return
    startTransition(async () => {
  ...
  return (
    <div className="flex items-center gap-1">
      <Sheet open={editOpen} onOpenChange={(o) => { setEditOpen(o); if (o) form.reset({ name }) }}>
