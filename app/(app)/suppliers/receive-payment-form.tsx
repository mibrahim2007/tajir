...
import { createSupplierRefundAction } from '@/app/actions/create-supplier-refund'
...
export function ReceivePaymentForm({ supplierId, today, nextSerial }: { supplierId: string; today: string; nextSerial?: string | null }) {
  ...
  const onSubmit = (values: FormValues) => {
    startTransition(async () => {
      setServerError(null)
      const result = await createSupplierRefundAction({ ...values, supplierId })
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent className="overflow-y-auto">
        <SheetHeader>
          <SheetTitle>Receive Payment from Supplier</SheetTitle>
          <SheetDescription>
            Record a refund, overpayment return, or reimbursement received from this supplier.
          </SheetDescription>
        </SheetHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} onKeyDown={handleEnterToNext} className="flex flex-col gap-4 mt-6">
            ...
            <FormField control={form.control} name="notes" render={({ field }) => (
              <FormItem>
                ...
                <FormControl><Input placeholder="e.g. Overpayment return for PO-0042…" {...field} /></FormControl>
                <FormMessage />
              </FormItem>
            )} />
