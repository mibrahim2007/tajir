...
export async function deleteProfitAllocationAction(input: unknown): Promise<ActionResult<void>> {
  ...
  const { data: alloc } = await admin
    ...
    .select('serial_number, period_start, period_end, net_profit')
    .eq('id', id)
  ...
  await admin.from('tajir_journal_entries').delete()
    .eq('tenant_id', tenantId).eq('source_type', 'profit_allocation').eq('source_id', id)
  ...
  await createAuditEntry({
    ...
    before: {
      ...
      periodEnd: alloc.period_end, netProfit: alloc.net_profit,
    },
  })
