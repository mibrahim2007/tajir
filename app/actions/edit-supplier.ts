...
export async function editSupplierAction(input: unknown): Promise<ActionResult<void>> {
  ...
  const { error } = await admin
    .from('suppliers')
    .update({ name, email: storedEmail })
    .eq('id', id)
    .eq('tenant_id', tenantId)

  if (error) return { success: false, error: 'Failed to update supplier', code: 'INTERNAL_ERROR' }

  await createAuditEntry({ tenantId, userId: user.id, action: 'update', entity: 'suppliers', entityId: id, before: { name: existing.name, email: existing.email }, after: { name, email: storedEmail } })
