...
import type { TajirCustomer } from '@/db/schema'
...
export async function createCustomerAction(input: unknown): Promise<ActionResult<TajirCustomer>> {
  ...
  const [customer] = await db.insert(tajirCustomers).values({
    tenantId, name,
