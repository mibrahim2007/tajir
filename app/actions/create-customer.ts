...
export async function createCustomerAction(input: unknown): Promise<ActionResult<{ id: string }>> {
  ...
  const { name, phone, openingBalance, openingBalanceCurrency, exchangeRate } = parsed.data
  ...
  await createAuditEntry({ tenantId, userId: user.id, action: 'create', entity: 'tajir_customers', entityId: customer.id, after: { name, phone: phone || null, openingBalance, openingBalanceCurrency } })
