...
export async function editSaleReturnAction(input: unknown): Promise<ActionResult<void>> {
  ...
  const { id, customerId, stockItemId, quantity, rate, currencyCode, exchangeRate, date, reason } = parsed.data
  ...
  const { data: existing } = await admin
    .from('sale_returns')
    .select('quantity, rate, pkr_equivalent, date, reason, stock_item_id, customer_id, currency_code')
    .eq('id', id)
