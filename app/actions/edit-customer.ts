...
export async function editCustomerAction(input: unknown): Promise<ActionResult<void>> {
  ...
  await createAuditEntry({ tenantId, userId: user.id, action: 'update', entity: 'tajir_customers', entityId: id, before: { name: existing.name, phone: existing.phone }, after: { name, phone: phone || null } })
